...
export function buildAgentPrompt({
  ...
  return [
    ...
    "只读取上面声明的 Skill 快照、assignment 和输入文件。不要读取 execution-plan.json、run-lock.json、evals.json、断言、answer key、其他实验臂或未在上面声明的历史输出。",
    "只在唯一可写目录中写入声明的输出；不要访问网络、发送消息、安装依赖、提交或推送 Git，除非权限声明明确允许。",
    "完整执行下面的用户任务（包括用户任务要求的完整 review）；但不要嵌套启动 Verify/Evolve 评测流程、不要派发子 Agent、不要启动 Dashboard，也不要声称批准或授权发布。",
    "若必须保留的输出包含 outputs/response.md，你的最终可见答复本身就是该产物：执行框架会把它原样保留为 outputs/response.md。",
