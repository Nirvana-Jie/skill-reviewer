...
def critical_issues_have_problem_why_fix(issues: list[str]) -> bool:
    ...
    label_groups = [
        ("problem", "问题"),
        ("why it matters", "为何重要"),
        ("suggested fix", "fix", "修复"),
    ]
    for issue in issues:
        if not all(has_explicit_label(issue, group) for group in label_groups):
            return False
    return True


def has_explicit_label(text: str, labels: tuple[str, ...]) -> bool:
    for label in labels:
        pattern = (
            rf"(?im)^\s*(?:\d+[.)、]\s*)?(?:[-*]\s*)?"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--contract",
        type=Path,
        default=skill_root / "evals" / "local-skill-review-snapshot.json",
...
def main(argv: list[str] | None = None) -> int:
    ...
    repo_root = args.contract.parent.parent
