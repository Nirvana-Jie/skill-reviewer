...
3. **先验证尺子，再评价候选**：必需文本 Oracle 要通过正反例校准，配对执行
   必须正确绑定；候选效果方向混合属于波动，不等于测量工具失效。
...
采样次数与确定性独立声明。旧协议仍默认确定性一次、随机性成对三次。三次是
有界治理默认值，不是统计置信度。系统保留每次配对结果：任一次退化都会阻塞
接受，随机性主要目标必须在每次配对中都达到实质提升阈值。配对方向冲突会使
候选被标记为波动，但不会让原本有效的测量失效，也不会通过多数票制造胜者；
任一次退化或主要目标未达阈值仍会由逐次门禁淘汰候选。
...
判定不是简单平均分：安全或触发可靠性触碰红线会直接阻塞。候选发布还必须
通过所有硬门禁、在每个已声明目标的每次配对中不超过退化容忍阈值，并让至少
一个主要目标在每次配对中达到预先声明的实质提升阈值。Dashboard 同时展示
方向归一化的平均差值和每次配对差值，但不会把它们描述成置信区间。
...
3. **候选是否真的更好？** 对比方向归一化的目标差值、每次配对结果、阈值、执行和文件改动。
4. **下一步做什么？** 只读展示状态机的 `next_action`、责任归因和人工边界，不在浏览器中另建工作流引擎。

评审总览是唯一的主结论入口；改动、运行和证据档案负责解释结论。Dashboard
严格只读，不提供 Agent 任务账本或写入路由。评分与发布真相仍由 Runtime 负责。
...
Dashboard 只解释下一状态，不会安排、唤醒或向 Agent 会话投递任务。需要继续时，
应在保留运行上下文的当前 Agent 任务或新任务中执行。
