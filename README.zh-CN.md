...
“执行链路”视图会把同一份 `evals.json` 真实运行证据按五段展开：锁定场景定义、主 Agent
分发、逐实验臂/逐 repeat 执行、确定性与补充语义检查、场景和发布门禁。每次执行都来自
投影出的状态、绑定错误数、执行摘要、断言计数和保留产物；异常同时使用叉号等形状语义，
不只依赖颜色。只有 Eval 清单、计划锁、执行配置、精确 repeat 覆盖和下游证据都能绑定时，
链路才显示“完整绑定”；否则会逐项列出可信度缺口。该视图展示可审计输入、分发、输出、
检查和决定，不展示或臆造模型的私有思维链。
